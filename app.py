...
# These weights are derived from the SHAP analysis of your RF and XGBoost models.
def calculate_risks(data):
    # Starting "Baseline" (Log-odds for ~2.2% mortality and ~24.4% morbidity)
    # These represent the average intercept of the models.
    mort_score = -4.3 
    morb_score = -1.1

    # --- DEMOGRAPHICS & ASA ---
    # Age impact: Per year above/below mean cohort age
    age_impact = (data['age'] - 65) * 0.04
    mort_score += age_impact
    morb_score += (data['age'] - 65) * 0.02

    # Race & Ethnicity (Minor SHAP weights)
    if data['race'] == "Black": mort_score += 0.15; morb_score += 0.1
    if data['hispanic'] == "Yes": morb_score += 0.1

    # ASA Class: Major driver for both models
    ...
    # --- SURGICAL FACTORS ---
    # Partial is the safest diversion/procedure type
    if data['procedure'] == "Partial Cystectomy":
        mort_score -= 1.4; morb_score -= 1.7
    # Neobladder increases morbidity complexity
    elif data['procedure'] == "Radical Cystectomy w/ Neobladder":
        mort_score += 0.1; morb_score += 0.5
    # Ileal Conduit is the baseline for Radical
    else:
        mort_score += 0.0; morb_score += 0.1

    if data['neoadj']: mort_score += 0.2
    if data['prior_rad']: mort_score += 0.4; morb_score += 0.4

    # --- LABS ---
    # Albumin (Major predictor)
    if data['alb'] < 3.5:
    ...
    # --- SIGMOID CONVERSION ---
    # Maps scores to actual probabilities (0.0 to 1.0)
...
st.write("© 2026 Research Validation Tool | Cystectomy ML Project")